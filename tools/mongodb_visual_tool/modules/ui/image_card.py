...
class ImageCard(ttk.Frame):
    ...
    def load_image(self):
        ...
        try:
            ...
            if not hasattr(self, 'image_label') or not self.image_label.winfo_exists():
                print("图像标签已被销毁，无法加载图像")
                return False
        except tk.TclError:
            print("UI组件已被销毁，无法加载图像")
            return False
        ...
        if not self.image_path:
            # 无图像路径
            try:
                self.image_label.configure(text="无图像路径")
            except tk.TclError:
                pass
            return False
        ...
        if not os.path.exists(self.image_path):
            # 打印详细信息以便调试
            print(f"图像不存在: {self.image_path}")
            try:
                self.image_label.configure(text=f"图像不存在\n{os.path.basename(self.image_path)}")
            except tk.TclError:
                pass
            return False
        ...
        try:
            ...
            # 打印详细错误信息
            import traceback
            print(f"加载图像失败: {self.image_path}")
            print(f"错误: {str(e)}")
            traceback.print_exc()
            
            try:
                if self.image_label.winfo_exists():
                    self.image_label.configure(text=f"加载失败:\n{str(e)}")
            except tk.TclError:
                pass
            return False
    ...
    def setup_context_menu(self, callback):
        ...
        self.image_frame.bind("<Button-3>", self._show_context_menu)
        self.meta_frame.bind("<Button-3>", self._show_context_menu)
